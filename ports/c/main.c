...
VV_LOCAL_SYMBOL  int compare_14897371507143444986_RepIndex_by_idx(RepIndex* a, RepIndex* b) {
...
VV_LOCAL_SYMBOL  int compare_6869001418358671327_string(string* a, string* b) {
...
VV_LOCAL_SYMBOL  int compare_6138052225949578915_int(int* a, int* b) {
	if (*a < *b) return -1;
	else return 1;
}
...
#define _const_main__line_sep 10
...
static int v_typeof_interface_idx_IError(int sidx) { /* IError */ 
	...
	if (sidx == _IError_os__ExecutableNotFoundError_index) return 141;
	return 30;
}
...
VNORETURN VV_LOCAL_SYMBOL void panic_debug(int line_no, string file, string mod, string fn_name, string s) {
	...
	#else
	{
		...
		eprintln( str_intp(2, _MOV((StrIntpData[]){{_SLIT("   v hash: "), 0xfe10, {.d_s = _SLIT("4475759")}}, {_SLIT0, 0, { .d_c = 0 }}})));
		eprintln(_SLIT("========================================="));
		#if defined(_VNATIVE)
		{
			exit(1);
...
string string_replace_each(string s, Array_string vals) {
	...
	if (idxs.len > 0) { qsort(idxs.data, idxs.len, idxs.element_size, (int (*)(const void *, const void *))&compare_14897371507143444986_RepIndex_by_idx); };
	u8* b = malloc_noscan((int)(new_len + 1));
	int idx_pos = 0;
...
strings__textscanner__TextScanner strings__textscanner__new(string input) {
	strings__textscanner__TextScanner _t1 = ((strings__textscanner__TextScanner){.input = /*clone1*/ string_clone_static(input),.ilen = input.len,.pos = 0,});
	return _t1;
}
...
Array_string os__args_after(string cut_word) {
	...
	if (!(Array_string_contains(_const_os__args, cut_word))) {
		array _sref467 = (cargs); // free array on re-assignment2
		cargs = array_clone_to_depth(&_const_os__args, 0);
		array_free(&_sref467);
	} else {
		bool found = false;
		array_push((array*)&cargs, _MOV((string[]){ string_clone(/*2*/string_clone((*(string*)array_get(_const_os__args, 0)))) }));
...
Array_string os__args_before(string cut_word) {
	...
	Array_string cargs = __new_array_with_default(0, 0, sizeof(string), 0);
	if (!(Array_string_contains(_const_os__args, cut_word))) {
		array _sref972 = (cargs); // free array on re-assignment2
		cargs = array_clone_to_depth(&_const_os__args, 0);
		array_free(&_sref972);
	} else {
		array_push((array*)&cargs, _MOV((string[]){ string_clone(/*2*/string_clone((*(string*)array_get(_const_os__args, 0)))) }));
...
Map_string_string os__environ(void) {
	...
	#else
	{
		...
		for (;;) {
			...
			i++;
			string_free(&eline); // autofreed var os false
}
	}
	#endif
	return res;
...
Array_string os__fd_slurp(int fd) {
	...
	for (;;) {
		...
		int b = mr_886.arg1;
		if (b <= 0) {
				string_free(&s); // autofreed var os false
break;
		}
...
__NOINLINE _result_os__File os__open_file(string path, string mode, Array_int options) {
	...
	_result_ok(&(os__File[]) { ((os__File){.cfile = cfile,.fd = fd,.is_opened = true,}) }, (_result*)(&_t6), sizeof(os__File));
		string_free(&p); // autofreed var os false
return _t6;
	string_free(&p); // autofreed var os false
}
...
_result_int os__File_read(os__File* f, Array_u8* buf) {
	...
	if (nbytes <= 0) {
		...
		if (ferror(((FILE*)(f->cfile))) != 0) {
			return (_result_int){ .is_error=true, .err=I_os__NotExpected_to_Interface_IError(((os__NotExpected*)memdup(&(os__NotExpected){.cause = /*clone1*/_SLIT("unexpected error from fread"),.code = -1,}, sizeof(os__NotExpected)))), .data={EMPTY_STRUCT_INITIALIZATION} };
		}
	}
	_result_int _t4;
...
Array_u8 os__File_read_bytes_at(os__File* f, int size, u64 pos) {
	...
	if (_t1.is_error) {
		...
		Array_u8 _t2 = __new_array_with_default(0, 0, sizeof(u8), 0);
			array_free(&arr); // autofreed var os false
return _t2;
	}
...
string os__abs_path(string path) {
	...
	if (string__eq(npath, _SLIT("."))) {
			string_free(&npath); // autofreed var os false
return wd;
	}
	if (!os__is_abs_path(npath)) {
		...
		string _t3 = os__norm_path(strings__Builder_str(&sb));
			strings__Builder_free(&sb); // autofreed var os false
	string_free(&npath); // autofreed var os false
return _t3;
		strings__Builder_free(&sb); // autofreed var os false
}
		string_free(&wd); // autofreed var os false
return npath;
	string_free(&wd); // autofreed var os false
}
...
return _const_os__dot_str;
	...
	if (!(Array_string_contains(spath, _SLIT("..")))) {
		string _t4; /* if prepend */
		if (volume_len != 0) {
			_t4 = string__plus(volume, cpath);
		} else {
			_t4 = cpath;
		}
		string _t3 =  _t4;
			Array_string_free(&spath); // autofreed var os false
	string_free(&cpath); // autofreed var os false
	string_free(&volume); // autofreed var os false
return _t3;
	}
	...
	for (int i = (int)(spath_len - 1); i >= 0; i--) {
		string part = /*2*/string_clone(((string*)spath.data)[i]);
		if ((part).len == 0) {
				string_free(&part); // autofreed var os false
continue;
		}
...
continue;
	...
	if (res.len == 0) {
		if (volume_len != 0) {
				string_free(&res); // autofreed var os false
	Array_string_free(&new_path); // autofreed var os false
	strings__Builder_free(&sb); // autofreed var os false
	Array_string_free(&spath); // autofreed var os false
	string_free(&cpath); // autofreed var os false
return volume;
...
return _const_os__path_separator;
	...
	if (volume_len != 0) {
		string _t8 = string__plus(volume, res);
			string_free(&res); // autofreed var os false
	Array_string_free(&new_path); // autofreed var os false
	strings__Builder_free(&sb); // autofreed var os false
	Array_string_free(&spath); // autofreed var os false
...
_result_string os__existing_path(string path) {
	...
	for (;;) {
		...
		if (os__is_slash(curr) && !os__is_slash(((u8)(peek)))) {
			string _sref3949 = (recent_path); // free string on re-assignment2
			recent_path = string_substr(path, 0, range);
			string_free(&_sref3949);
			continue;
		}
		if (!os__is_slash(curr) && (peek == -1 || os__is_slash(((u8)(peek))))) {
			string curr_path = string_substr(path, 0, range);
			if (os__exists(curr_path)) {
				string _sref4114 = (recent_path); // free string on re-assignment2
				recent_path =  string_clone_static(curr_path);
				string_free(&_sref4114);
					string_free(&curr_path); // autofreed var os false
continue;
			}
...
break;
			...
			_result_ok(&(string[]) { recent_path }, (_result*)(&_t3), sizeof(string));
				string_free(&curr_path); // autofreed var os false
	string_free(&recent_path); // autofreed var os false
	strings__textscanner__TextScanner_free(&sc); // autofreed var os false
return _t3;
			string_free(&curr_path); // autofreed var os false
}
	}
...
_result_void os__rename(string src, string dst) {
	...
	if (os__is_dir(rdst)) {
		string _sref5408 = (rdst); // free string on re-assignment2
	...
	#endif
	return (_result_void){0};
	string_free(&rdst); // autofreed var os false
}
...
_result_FILE_ptr os__vfopen(string path, string mode) {
	...
	#endif
	if (isnil(fp)) {
		return (_result_FILE_ptr){ .is_error=true, .err=os__error_posix(((os__SystemError){.msg = /*clone1*/ str_intp(2, _MOV((StrIntpData[]){{_SLIT("failed to open file \""), 0xfe10, {.d_s = path}}, {_SLIT("\""), 0, { .d_c = 0 }}})),.code = _const_os__error_code_not_set,})), .data={EMPTY_STRUCT_INITIALIZATION} };
	} else {
...
_result_void os__rm(string path) {
	...
	#endif
	if (rc == -1) {
		return (_result_void){ .is_error=true, .err=os__error_posix(((os__SystemError){.msg = /*clone1*/string__plus( str_intp(2, _MOV((StrIntpData[]){{_SLIT("Failed to remove \""), 0xfe10, {.d_s = path}}, {_SLIT("\": "), 0, { .d_c = 0 }}})), os__posix_get_error_msg(errno)),.code = _const_os__error_code_not_set,})), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	return (_result_void){0};
}
...
VV_LOCAL_SYMBOL void os__print_c_errno(void) {
	...
	string_free(&se); // autofreed var os false
}
...
string os__get_raw_line(void) {
	...
	#else
	{
		...
		string str = tos(((u8*)(buf)),  _t2);
		string ret = string_clone(str);
			string_free(&str); // autofreed var os false
return ret;
		string_free(&str); // autofreed var os false
}
	#endif
...
Array_u8 os__get_raw_stdin(void) {
	...
	#else
	{
		...
		Array_u8 _t2 = ((array){.data = ((voidptr)(buf)),.offset = 0,.len =  _t3,.cap = ((int)(max)),.flags = 0,.element_size = 1,});
		return _t2;
	}
	#endif
...
_result_void os__chmod(string path, int mode) {
	if (chmod(((char*)(path.str)), mode) != 0) {
		return (_result_void){ .is_error=true, .err=os__error_posix(((os__SystemError){.msg = /*clone1*/string__plus(_SLIT("chmod failed: "), os__posix_get_error_msg(errno)),.code = _const_os__error_code_not_set,})), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	return (_result_void){0};
}
...
_result_os__File os__open_append(string path) {
	...
	#endif
	if (isnil(file.cfile)) {
		return (_result_os__File){ .is_error=true, .err=os__error_posix(((os__SystemError){.msg = /*clone1*/ str_intp(2, _MOV((StrIntpData[]){{_SLIT("failed to create(append) file \""), 0xfe10, {.d_s = path}}, {_SLIT("\""), 0, { .d_c = 0 }}})),.code = _const_os__error_code_not_set,})), .data={EMPTY_STRUCT_INITIALIZATION} };
	}
	file.is_opened = true;
	_result_os__File _t3;
...
_result_void os__execvp(string cmdpath, Array_string cmdargs) {
	...
	for (int i = 0; i < cmdargs.len; ++i) {
		array_push((array*)&cargs, _MOV((char*[]){ ((char*)(/*2*/string_clone((*(string*)array_get(cmdargs, i))).str)) }));
	}
	...
	VUNREACHABLE();
	return (_result_void){0};
	array_free(&cargs); // autofreed var os false
}
...
_result_void os__execve(string cmdpath, Array_string cmdargs, Array_string envs) {
	...
	for (int i = 0; i < cmdargs.len; ++i) {
		array_push((array*)&cargv, _MOV((char*[]){ ((char*)(/*2*/string_clone((*(string*)array_get(cmdargs, i))).str)) }));
	}
	for (int i = 0; i < envs.len; ++i) {
		array_push((array*)&cenvs, _MOV((char*[]){ ((char*)(/*2*/string_clone((*(string*)array_get(envs, i))).str)) }));
	}
	...
	return (_result_void){0};
	array_free(&cenvs); // autofreed var os false
	array_free(&cargv); // autofreed var os false
}
...
_result_Array_string os__glob(Array_string patterns) {
 ...
 ;
	...
	if (matches.len > 0) { qsort(matches.data, matches.len, matches.element_size, (int (*)(const void *, const void *))&compare_6869001418358671327_string); };
	_result_Array_string _t4;
	_result_ok(&(Array_string[]) { matches }, (_result*)(&_t4), sizeof(Array_string));
		Array_string_free(&matches); // autofreed var os false
return _t4;
	Array_string_free(&matches); // autofreed var os false
}
...
IError os__last_error(void) {
	...
	#else
	{
		...
		IError _t2 = error_with_code(msg, code);
			string_free(&msg); // autofreed var os false
return _t2;
		string_free(&msg); // autofreed var os false
}
	#endif
...
inline IError os__error_posix(os__SystemError e) {
	...
	string _t2; /* if prepend */
	if ((e.msg).len == 0) {
		_t2 = os__posix_get_error_msg(code);
	} else {
		_t2 = e.msg;
	}
	string message =  _t2;
	IError _t3 = error_with_code(message, code);
		string_free(&message); // autofreed var os false
return _t3;
	string_free(&message); // autofreed var os false
}
...
VV_LOCAL_SYMBOL string os__executable_fallback(void) {
	...
	string exepath = /*2*/string_clone((*(string*)array_get(_const_os__args, 0)));
	if (!os__is_abs_path(exepath)) {
		...
		if (string_contains(rexepath, _const_os__path_separator)) {
			string _sref1364 = (exepath); // free string on re-assignment2
			exepath = os__join_path_single(_const_os__wd_at_startup, exepath);
			string_free(&_sref1364);
		} else {
			...
			if ((foundpath).len != 0) {
				string _sref1580 = (exepath); // free string on re-assignment2
				exepath =  string_clone_static(foundpath);
				string_free(&_sref1580);
			}
		}
		string_free(&rexepath); // autofreed var os false
	string_free(&other_separator); // autofreed var os false
}
	string _sref1613 = (exepath); // free string on re-assignment2
	exepath = os__real_path(exepath);
	string_free(&_sref1613);
	return exepath;
}
...
_result_void os__cp_all(string src, string dst, bool overwrite) {
	...
	if (!os__is_dir(source_path)) {
		...
		if (os__is_dir(dest_path)) {
			_t2 = os__join_path_single(dest_path, fname);
		} else {
			_t2 = dest_path;
		}
		string adjusted_path =  _t2;
		if (os__exists(adjusted_path)) {
			if (overwrite) {
				_result_void _t3 = os__rm(adjusted_path);
				if (_t3.is_error) {
					_result_void _t4;
					memcpy(&_t4, &_t3, sizeof(_result));
					return _t4;
 ...
 ;
		...
		_result_void _t6 = os__cp(source_path, adjusted_path);
		if (_t6.is_error) {
			_result_void _t7;
			memcpy(&_t7, &_t6, sizeof(_result));
			return _t7;
 ...
 ;
			string_free(&adjusted_path); // autofreed var os false
	string_free(&fname); // autofreed var os false
return (_result_void){0};
		string_free(&adjusted_path); // autofreed var os false
	string_free(&fname); // autofreed var os false
}
	if (!os__exists(dest_path)) {
		_result_void _t8 = os__mkdir(dest_path, ((os__MkdirParams){.mode = 0777,}));
		if (_t8.is_error) {
			_result_void _t9;
 ...
 ;
	...
	_result_Array_string _t11 = os__ls(source_path);
	if (_t11.is_error) {
		_result_void _t12;
 	...
 	Array_string files =  (*(Array_string*)_t11.data);
	for (int _t13 = 0; _t13 < files.len; ++_t13) {
		string file = ((string*)files.data)[_t13];
		string sp = os__join_path_single(source_path, file);
		string dp = os__join_path_single(dest_path, file);
		if (os__is_dir(sp)) {
			if (!os__exists(dp)) {
				_result_void _t14 = os__mkdir(dp, ((os__MkdirParams){.mode = 0777,}));
				if (_t14.is_error) {
					_result_void _t15;
 ...
 ;
		...
		_result_void _t16 = os__cp_all(sp, dp, overwrite);
		if (_t16.is_error) {
			IError err = _t16.err;
			_result_void _t17 = os__rmdir(dp);
			if (_t17.is_error) {
				IError err = _t17.err;
 ...
 ;
		string_free(&dp); // autofreed var os false
	string_free(&sp); // autofreed var os false
}
	return (_result_void){0};
	Array_string_free(&files); // autofreed var os false
	string_free(&dest_path); // autofreed var os false
	string_free(&source_path); // autofreed var os false
}
...
_result_void os__rmdir_all(string path) {
	...
	for (int _t3 = 0; _t3 < items.len; ++_t3) {
		...
		if (os__is_dir(fullpath) && !os__is_link(fullpath)) {
			...
			if (_t4.is_error) {
				IError err = _t4.err;
				string _sref5265 = (ret_err); // free string on re-assignment2
				ret_err = IError_name_table[err._typ]._method_msg(err._object);
				string_free(&_sref5265);
			}
 ...
 ;
			...
			if (_t5.is_error) {
				IError err = _t5.err;
				string _sref5319 = (ret_err); // free string on re-assignment2
				ret_err = IError_name_table[err._typ]._method_msg(err._object);
				string_free(&_sref5319);
			}
			
 ;
		}
		string_free(&fullpath); // autofreed var os false
}
	_result_void _t6 = os__rmdir(path);
	if (_t6.is_error) {
		IError err = _t6.err;
		string _sref5366 = (ret_err); // free string on re-assignment2
		ret_err = IError_name_table[err._typ]._method_msg(err._object);
		string_free(&_sref5366);
	}
 ...
 ;
	...
	return (_result_void){0};
	Array_string_free(&items); // autofreed var os false
	string_free(&ret_err); // autofreed var os false
}
...
string os__file_ext(string opath) {
	...
	if (pos == -1) {
		string _t2 = _SLIT("");
			string_free(&path); // autofreed var os false
return _t2;
	}
...
string os__dir(string opath) {
	...
	_option_int _t3 = string_last_index(path, _const_os__path_separator);
	if (_t3.state != 0) {
		IError err = _t3.err;
		string _t4 = _SLIT(".");
			string_free(&path); // autofreed var os false
...
return _t5;
	...
	string _t6 = string_substr(path, 0, pos);
		string_free(&path); // autofreed var os false
	string_free(&other_separator); // autofreed var os false
return _t6;
	string_free(&path); // autofreed var os false
...
return _const_os__path_separator;
	...
	if (string_ends_with(path, _const_os__path_separator)) {
		...
		_option_int _t4 = string_last_index(path2, _const_os__path_separator);
		if (_t4.state != 0) {
			IError err = _t4.err;
			string _t5 = string_clone(path2);
				string_free(&path2); // autofreed var os false
	string_free(&path); // autofreed var os false
return _t5;
		}
		
 		int pos =  (*(int*)_t4.data);
		string _t6 = string_substr(path2, (int)(pos + 1), 2147483647);
			string_free(&path2); // autofreed var os false
	string_free(&path); // autofreed var os false
return _t6;
		string_free(&path2); // autofreed var os false
}
	_option_int _t7 = string_last_index(path, _const_os__path_separator);
	if (_t7.state != 0) {
		IError err = _t7.err;
		string _t8 = string_clone(path);
			string_free(&path); // autofreed var os false
return _t8;
	}
	
 	int pos =  (*(int*)_t7.data);
	string _t9 = string_substr(path, (int)(pos + 1), 2147483647);
		string_free(&path); // autofreed var os false
	string_free(&other_separator); // autofreed var os false
return _t9;
	string_free(&path); // autofreed var os false
...
string os__file_name(string opath) {
	...
	string other_separator =  _t1;
	string path = string_replace(opath, other_separator, _const_os__path_separator);
	string _t2 = string_all_after_last(path, _const_os__path_separator);
		string_free(&path); // autofreed var os false
	string_free(&other_separator); // autofreed var os false
return _t2;
	string_free(&path); // autofreed var os false
...
Array_string os__get_lines(void) {
	...
	for (;;) {
		string _sref8856 = (line); // free string on re-assignment2
		line = os__get_line();
		string_free(&_sref8856);
		if (line.len <= 0) {
			break;
		}
		string _sref8910 = (line); // free string on re-assignment2
		line = string_trim_space(line);
		string_free(&_sref8910);
		array_push((array*)&inputstr, _MOV((string[]){ string_clone(line) }));
	}
		string_free(&line); // autofreed var os false
return inputstr;
	string_free(&line); // autofreed var os false
}
...
string os__get_lines_joined(void) {
	...
	for (;;) {
		string _sref9175 = (line); // free string on re-assignment2
		line = os__get_line();
		string_free(&_sref9175);
		if (line.len <= 0) {
			break;
		}
		string _sref9229 = (line); // free string on re-assignment2
		line = string_trim_space(line);
		string_free(&_sref9229);
		inputstr = string__plus(inputstr,  string_clone_static(line));
	}
		string_free(&line); // autofreed var os false
return inputstr;
	string_free(&line); // autofreed var os false
...
string os__get_raw_lines_joined(void) {
	...
	for (;;) {
		string _sref9649 = (line); // free string on re-assignment2
		line = os__get_raw_line();
		string_free(&_sref9649);
		if (line.len <= 0) {
			break;
		}
	...
	string res = Array_string_join(lines, _SLIT(""));
		Array_string_free(&lines); // autofreed var os false
	string_free(&line); // autofreed var os false
return res;
	Array_string_free(&lines); // autofreed var os false
	string_free(&line); // autofreed var os false
...
_result_Array_string os__user_names(void) {
	...
	#else
	{
		...
		_result_ok(&(Array_string[]) { users }, (_result*)(&_t7), sizeof(Array_string));
			Array_string_free(&users); // autofreed var os false
	Array_string_free(&lines); // autofreed var os false
return _t7;
		Array_string_free(&users); // autofreed var os false
	Array_string_free(&lines); // autofreed var os false
	...
	for (int _t2 = 0; _t2 < _const_os__executable_suffixes.len; ++_t2) {
		...
		if (os__is_abs_path(fexepath)) {
			...
			_result_ok(&(string[]) { os__real_path(fexepath) }, (_result*)(&_t3), sizeof(string));
				string_free(&fexepath); // autofreed var os false
return _t3;
		}
		string res = _SLIT("");
		...
		for (int _t4 = 0; _t4 < paths.len; ++_t4) {
			...
			if (os__exists(found_abs_path) && os__is_executable(found_abs_path)) {
				string _sref12981 = (res); // free string on re-assignment2
				res =  string_clone_static(found_abs_path);
				string_free(&_sref12981);
					string_free(&found_abs_path); // autofreed var os false
break;
			}
			string_free(&found_abs_path); // autofreed var os false
}
		if (res.len > 0) {
			...
			_result_ok(&(string[]) { os__real_path(res) }, (_result*)(&_t5), sizeof(string));
				Array_string_free(&paths); // autofreed var os false
	string_free(&path); // autofreed var os false
	string_free(&res); // autofreed var os false
	string_free(&fexepath); // autofreed var os false
return _t5;
...
VV_LOCAL_SYMBOL void os__impl_walk_ext(string path, string ext, Array_string* out) {
	...
	if (string_ends_with(path, _const_os__path_separator)) {
		...
		_t2 = _const_os__path_separator;
	}
	string separator =  _t2;
	...
	string_free(&separator); // autofreed var os false
}
...
continue;
		...
		for (int idx = (int)(files.len - 1); idx >= 0; idx--) {
			array_push((array*)&remaining, _MOV((string[]){ string_clone(string__plus(string__plus(cpath, _SLIT("/")), /*2*/string_clone((*(string*)array_get(files, idx))))) }));
		}
		string_free(&cpath); // autofreed var os false
}
	string_free(&clean_path); // autofreed var os false
	Array_string_free(&remaining); // autofreed var os false
}
...
_result_void os__mkdir_all(string opath, os__MkdirParams params) {
 ...
 ;
	...
	return (_result_void){0};
	Array_string_free(&path_parts); // autofreed var os false
	string_free(&p); // autofreed var os false
	string_free(&path); // autofreed var os false
	string_free(&other_separator); // autofreed var os false
}
...
VV_LOCAL_SYMBOL void os__create_folder_when_it_does_not_exist(string path) {
	...
	if (_t1.is_error) {
		...
		string _t2 = IError_str(err); _v_panic(_t2); string_free(&_t2);
		;
		VUNREACHABLE();
	;
	}
...
string os__cache_dir(void) {
	...
	if (xdg_cache_home.len > 0) {
		...
		_t1 = os__join_path_single(os__home_dir(), _SLIT(".cache"));
	}
	string cdir =  _t1;
	os__create_folder_when_it_does_not_exist(cdir);
		string_free(&xdg_cache_home); // autofreed var os false
return cdir;
	string_free(&xdg_cache_home); // autofreed var os false
}
...
return _t1;
	if ((path).len == 0) {
		string _sref21800 = (path); // free string on re-assignment2
		path = _SLIT("/tmp");
		string_free(&_sref21800);
	}
	return path;
}
...
VV_LOCAL_SYMBOL string os__default_vmodules_path(void) {
	...
	string res = os__join_path_single(hdir, _SLIT(".vmodules"));
		string_free(&hdir); // autofreed var os false
return res;
	string_free(&hdir); // autofreed var os false
}
...
string os__vmodules_dir(void) {
	...
	if (paths.len > 0) {
		string _t1 = /*2*/string_clone((*(string*)array_get(paths, 0)));
			Array_string_free(&paths); // autofreed var os false
return _t1;
	}
...
Array_string os__vmodules_paths(void) {
	...
	if ((path).len == 0) {
		string _sref22895 = (path); // free string on re-assignment2
		path = os__default_vmodules_path();
		string_free(&_sref22895);
	}
	...
	for (int i = 0; i < splitted.len; ++i) {
		string si = /*2*/string_clone((*(string*)array_get(splitted, i)));
		string trimmed = string_trim_right(si, _const_os__path_separator);
		array_push((array*)&list, _MOV((string[]){ string_clone(trimmed) }));
		string_free(&trimmed); // autofreed var os false
	string_free(&si); // autofreed var os false
}
	Array_string _t2 = list;
		...
		// Defer end
		Array_string_free(&splitted); // autofreed var os false
	string_free(&path); // autofreed var os false
return _t2;
	Array_string_free(&splitted); // autofreed var os false
	string_free(&path); // autofreed var os false
...
_result_os__Result os__execute_opt(string cmd) {
	...
	_result_ok(&(os__Result[]) { res }, (_result*)(&_t2), sizeof(os__Result));
		os__Result_free(&res); // autofreed var os false
return _t2;
	os__Result_free(&res); // autofreed var os false
}
	...
	#elif defined(__APPLE__) || defined(__DARWIN__) || defined(__TARGET_IOS__)
	{
		...
		if ((home).len != 0) {
			...
			_result_ok(&(string[]) { string__plus(home, _SLIT("/Library/Application Support")) }, (_result*)(&_t2), sizeof(string));
				string_free(&home); // autofreed var os false
return _t2;
		}
		string_free(&home); // autofreed var os false
}
...
VV_LOCAL_SYMBOL Array_string os__glob_match(string dir, string pattern, string next_pattern, Array_string* matches) {
	...
	string pat =  string_clone_static(pattern);
	if (string__eq(pat, _SLIT("*"))) {
		...
		if (!string__eq(next_pattern, pattern) && (next_pattern).len != 0) {
			for (int _t4 = 0; _t4 < files.len; ++_t4) {
				...
				string_free(&pat); // autofreed var os false
return subdirs;
		}
	}
	if (string__eq(pat, _SLIT("**"))) {
		array _sref2498 = (files); // free array on re-assignment2
		files = os__walk_ext(dir, _SLIT(""));
		array_free(&_sref2498);
		string _sref2524 = (pat); // free string on re-assignment2
		pat =  string_clone_static(next_pattern);
		string_free(&_sref2524);
	}
	if (string_starts_with(pat, _SLIT("*"))) {
		mode = os__GlobMatch__ends_with;
		string _sref2595 = (pat); // free string on re-assignment2
		pat = string_substr(pat, 1, 2147483647);
		string_free(&_sref2595);
	}
	if (string_ends_with(pat, _SLIT("*"))) {
		os__GlobMatch _t7; /* if prepend */
		if (mode == os__GlobMatch__ends_with) {
			_t7 = os__GlobMatch__contains;
		} else {
			_t7 = os__GlobMatch__starts_with;
		}
		mode =  _t7;
		string _sref2725 = (pat); // free string on re-assignment2
		pat = string_substr(pat, 0, (int)(pat.len - 1));
		string_free(&_sref2725);
	}
	if (string_contains(pat, _SLIT("*"))) {
		mode = os__GlobMatch__start_and_ends_with;
	}
	for (int _t8 = 0; _t8 < files.len; ++_t8) {
		string file = ((string*)files.data)[_t8];
		string fpath =  string_clone_static(file);
		string _t9; /* if prepend */
		if (string_contains(file, _const_os__path_separator)) {
			Array_string pathwalk = string_split(file, _const_os__path_separator);
			_t9 = /*2*/string_clone((*(string*)array_get(pathwalk, (int)(pathwalk.len - 1))));
			Array_string_free(&pathwalk); // autofreed var os false
} else {
			string _t10; /* if prepend */
			if (string__eq(dir, _SLIT("."))) {
				_t10 = file;
			} else {
				_t10 =  str_intp(3, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = dir}}, {_SLIT("/"), 0xfe10, {.d_s = file}}, {_SLIT0, 0, { .d_c = 0 }}}));
			}
			string _sref2987 = (fpath); // free string on re-assignment2
			fpath =  _t10;
			string_free(&_sref2987);
			_t9 = file;
		}
		string f =  _t9;
		if ((string__eq(f, _SLIT(".")) || string__eq(f, _SLIT(".."))) || (f).len == 0) {
				string_free(&fpath); // autofreed var os false
continue;
		}
		bool _t11 = 0;
		...
		if (mode == (os__GlobMatch__any)) {
			...
			_t11 = string__eq(f, pat);
		}
		else if (mode == (os__GlobMatch__starts_with)) {
			_t11 = string_starts_with(f, pat);
		}
		else if (mode == (os__GlobMatch__ends_with)) {
			_t11 = string_ends_with(f, pat);
		}
		else if (mode == (os__GlobMatch__start_and_ends_with)) {
			Array_string p = string_split(pat, _SLIT("*"));
			_t11 = string_starts_with(f, /*2*/string_clone((*(string*)array_get(p, 0)))) && string_ends_with(f, /*2*/string_clone((*(string*)array_get(p, 1))));
			Array_string_free(&p); // autofreed var os false
}
		else if (mode == (os__GlobMatch__contains)) {
			_t11 = string_contains(f, pat);
		}bool hit = _t11;
		if (hit) {
			if (os__is_dir(fpath)) {
		...
		string_free(&f); // autofreed var os false
	string_free(&fpath); // autofreed var os false
}
		string_free(&pat); // autofreed var os false
return subdirs;
...
VV_LOCAL_SYMBOL _result_void os__native_glob_pattern(string pattern, Array_string* matches) {
	...
	string _t1; /* if prepend */
	if (string_starts_with(pattern, _const_os__path_separator)) {
		_t1 = _const_os__path_separator;
	} else {
		_t1 = _SLIT(".");
	}
	string cwd =  _t1;
	...
	for (int i = 0; i < steps.len; i++) {
		string step = /*2*/string_clone((*(string*)array_get(steps, i)));
		string _t2; /* if prepend */
		if ((int)(i + 1) == steps.len) {
			_t2 = step;
		} else {
			_t2 = /*2*/string_clone((*(string*)array_get(steps, (int)(i + 1))));
		}
		string step2 =  _t2;
		if ((step).len == 0) {
				string_free(&step); // autofreed var os false
continue;
		}
		if (os__is_dir( str_intp(4, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = cwd}}, {_SLIT0, 0xfe10, {.d_s = _const_os__path_separator}}, {_SLIT0, 0xfe10, {.d_s = step}}, {_SLIT0, 0, { .d_c = 0 }}})))) {
			string _t3; /* if prepend */
			if (string__eq(cwd, _SLIT("/"))) {
				_t3 = step;
			} else {
				string _t4; /* if prepend */
				if (string__eq(cwd, _SLIT(".")) || (cwd).len == 0) {
					_t4 = step;
				} else {
					string _t5; /* if prepend */
					if (string__eq(step, _SLIT(".")) || string__eq(step, _SLIT("/"))) {
						_t5 = cwd;
					} else {
						_t5 =  str_intp(3, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = cwd}}, {_SLIT("/"), 0xfe10, {.d_s = step}}, {_SLIT0, 0, { .d_c = 0 }}}));
					}
					_t4 =  _t5;
				}
				_t3 =  _t4;
			...
			string_free(&dd); // autofreed var os false
}
		Array_string subs = __new_array_with_default(0, 0, sizeof(string), 0);
		for (int _t7 = 0; _t7 < subdirs.len; ++_t7) {
			string sd = ((string*)subdirs.data)[_t7];
			...
			if (string__eq(cwd, _SLIT("/"))) {
				...
				if (string__eq(cwd, _SLIT(".")) || (cwd).len == 0) {
					...
					string _t10; /* if prepend */
					if (string__eq(sd, _SLIT(".")) || string__eq(sd, _SLIT("/"))) {
						_t10 = cwd;
					} else {
						_t10 =  str_intp(3, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = cwd}}, {_SLIT("/"), 0xfe10, {.d_s = sd}}, {_SLIT0, 0, { .d_c = 0 }}}));
					}
					_t9 =  _t10;
				}
				_t8 =  _t9;
			...
			string d =  _t8;
			_PUSH_MANY(&subs, (os__glob_match(string_replace(d, _SLIT("//"), _SLIT("/")), step, step2, matches)), _t11, Array_string);
			string_free(&d); // autofreed var os false
}
		array _sref4621 = (subdirs); // free array on re-assignment2
		subdirs = array_clone_to_depth(&subs, 0);
		array_free(&_sref4621);
		Array_string_free(&subs); // autofreed var os false
	string_free(&step2); // autofreed var os false
	string_free(&step); // autofreed var os false
}
	return (_result_void){0};
	Array_string_free(&subdirs); // autofreed var os false
	string_free(&cwd); // autofreed var os false
	Array_string_free(&steps); // autofreed var os false
}
...
os__Uname os__uname(void) {
	...
	{ // Unsafe block
		...
		if (uname(d) == 0) {
			string _sref5778 = (u.sysname); // free string on re-assignment2
			u.sysname = cstring_to_vstring(d->sysname);
			string_free(&_sref5778);
			string _sref5824 = (u.nodename); // free string on re-assignment2
			u.nodename = cstring_to_vstring(d->nodename);
			string_free(&_sref5824);
			string _sref5870 = (u.release); // free string on re-assignment2
			u.release = cstring_to_vstring(d->release);
			string_free(&_sref5870);
			string _sref5915 = (u.version); // free string on re-assignment2
			u.version = cstring_to_vstring(d->version);
			string_free(&_sref5915);
			string _sref5960 = (u.machine); // free string on re-assignment2
			u.machine = cstring_to_vstring(d->machine);
			string_free(&_sref5960);
		}
		_v_free(d);
	}
	return u;
...
_result_string os__hostname(void) {
	...
	if (gethostname(buf, size) == 0) {
		string _sref6172 = (hstnme); // free string on re-assignment2
		hstnme = cstring_to_vstring(buf);
		string_free(&_sref6172);
		_v_free(buf);
		_result_string _t1;
		_result_ok(&(string[]) { hstnme }, (_result*)(&_t1), sizeof(string));
			string_free(&hstnme); // autofreed var os false
return _t1;
	}
...
_result_Array_string os__ls(string path) {
	...
	_result_ok(&(Array_string[]) { res }, (_result*)(&_t4), sizeof(Array_string));
		Array_string_free(&res); // autofreed var os false
return _t4;
	Array_string_free(&res); // autofreed var os false
}
...
_result_void os__mkdir(string path, os__MkdirParams params) {
	...
	return (_result_void){0};
	string_free(&apath); // autofreed var os false
}
...
VV_LOCAL_SYMBOL int os__Process__spawn(os__Process* p) {
	if (!p->env_is_custom) {
		array _sref2332 = (p->env); // free array on re-assignment2
		p->env = __new_array_with_default(0, 0, sizeof(string), 0);
		array_free(&_sref2332);
...
os__Process* os__new_process(string filename) {
	os__Process* _t1 = ((os__Process*)memdup(&(os__Process){.filename = /*clone1*/ string_clone_static(filename),.pid = 0,.code = -1,.status = os__ProcessState__not_started,.err = (string){.str=(byteptr)"", .is_lit=1},.args = __new_array(0, 0, sizeof(string)),.work_folder = (string){.str=(byteptr)"", .is_lit=1},.env_is_custom = 0,.env = __new_array(0, 0, sizeof(string)),.use_stdio_ctl = 0,.use_pgroup = 0,.stdio_fd = {-1, -1, -1},.wdata = 0,.create_no_window = 0,}, sizeof(os__Process)));
	return _t1;
}
...
void os__Process_set_args(os__Process* p, Array_string pargs) {
	...
	array _sref2500 = (p->args); // free array on re-assignment2
	p->args =  array_clone_static_to_depth(pargs, 0);
	array_free(&_sref2500);
	return;
}
...
void os__Process_set_work_folder(os__Process* p, string path) {
	...
	string _sref2792 = (p->work_folder); // free string on re-assignment2
	p->work_folder = os__real_path(path);
	string_free(&_sref2792);
	return;
}
...
void os__Process_set_environment(os__Process* p, Map_string_string envs) {
	...
	array _sref3043 = (p->env); // free array on re-assignment2
	p->env = __new_array_with_default(0, 0, sizeof(string), 0);
	array_free(&_sref3043);
...
VV_LOCAL_SYMBOL int os__Process_unix_spawn_process(os__Process* p) {
	...
	if ((p->work_folder).len != 0) {
		if (!os__is_abs_path(p->filename)) {
			string _sref1983 = (p->filename); // free string on re-assignment2
			p->filename = os__abs_path(p->filename);
			string_free(&_sref1983);
		}
 ...
 ;
	...
	if (_t3.is_error) {
		IError err = _t3.err;
		string _t4 = IError_str(err); eprintln(_t4); string_free(&_t4);
		;
		_v_exit(1);
		VUNREACHABLE();
	;
...
VV_LOCAL_SYMBOL bool os__Process_impl_check_pid_status(os__Process* p, bool exit_early_on_ret0, int waitpid_options) {
	...
	p->code = ret;
	if (ret == -1) {
		string _sref2882 = (p->err); // free string on re-assignment2
		p->err = os__posix_get_error_msg(errno);
		string_free(&_sref2882);
		bool _t2 = false;
		return _t2;
	}
	...
	if (is_signaled) {
		p->status = os__ProcessState__aborted;
		string _sref3095 = (p->err); // free string on re-assignment2
...
_result_void os__open_uri(string uri) {
	...
	if ((vopen_uri_cmd).len == 0) {
		...
		{
			string _sref177 = (vopen_uri_cmd); // free string on re-assignment2
			vopen_uri_cmd = _SLIT("open");
			string_free(&_sref177);
		}
	...
	return (_result_void){0};
	os__Result_free(&result); // autofreed var os false
	string_free(&vopen_uri_cmd); // autofreed var os false
}
...
// TypeDecl
VV_LOCAL_SYMBOL void main__main(void) {
	println(_SLIT("BED - v0.0.1 - C"));
	Map_int_string buffer = new_map(sizeof(int), sizeof(string), &map_hash_int_4, &map_eq_int_4, &map_clone_int_4, &map_free_nop)
	;
	for (;;) {
string _arg_expr_split_0_155 = os__input(_SLIT("> "));// new af2 pre/* inserted before */
				Array_string userinput = string_split(/*af receiver arg*/_arg_expr_split_0_155, _SLIT(" "));
		if (string_is_int(/*2*/string_clone((*(string*)array_get(userinput, 0))))) {
			map_set(&buffer, &(int[]){string_int(/*2*/string_clone(((string*)userinput.data)[0]))}, &(string[]) { Array_string_join(array_slice(userinput, 1, userinput.len), _SLIT(" ")) });
		} else {
			string _t1 = string_to_upper(/*2*/string_clone(((string*)userinput.data)[0]));
			
			if (string__eq(_t1, _SLIT("LIST"))) {
				if (userinput.len != 1) {
string _arg_expr_println_1_360 = str_intp(3, _MOV((StrIntpData[]){{_SLIT("  "), 0xfe10, {.d_s = /*2*/string_clone((*(string*)array_get(userinput, 1)))}}, {_SLIT(" "), 0xfe10, {.d_s = (*(string*)map_get(ADDR(map, buffer), &(int[]){string_int(/*2*/string_clone(((string*)userinput.data)[1]))}, &(string[]){ (string){.str=(byteptr)"", .is_lit=1} }))}}, {_SLIT0, 0, { .d_c = 0 }}}));// new af2 pre/* inserted before */
										println(/*autofree arg*/_arg_expr_println_1_360);
					string_free(&_arg_expr_println_1_360); // autofreed var main false
} else {
					Array_int keys = map_keys(&buffer);
					if (keys.len > 0) { qsort(keys.data, keys.len, keys.element_size, (int (*)(const void *, const void *))&compare_6138052225949578915_int); };
					for (int _t2 = 0; _t2 < keys.len; ++_t2) {
						int i = ((int*)keys.data)[_t2];
						if (((*(string*)map_get(ADDR(map, buffer), &(int[]){i}, &(string[]){ (string){.str=(byteptr)"", .is_lit=1} }))).len != 0) {
string _arg_expr_println_1_541 = str_intp(3, _MOV((StrIntpData[]){{_SLIT("  "), 0xfe07, {.d_i32 = i}}, {_SLIT(" "), 0xfe10, {.d_s = (*(string*)map_get(ADDR(map, buffer), &(int[]){i}, &(string[]){ (string){.str=(byteptr)"", .is_lit=1} }))}}, {_SLIT0, 0, { .d_c = 0 }}}));// new af2 pre/* inserted before */
														println(/*autofree arg*/_arg_expr_println_1_541);
							string_free(&_arg_expr_println_1_541); // autofreed var main false
}
					}
					array_free(&keys); // autofreed var main false
}
			}
			else if (string__eq(_t1, _SLIT("LOAD"))) {
string _arg_expr_os_read_lines_1_634 = /*2*/string_clone((*(string*)array_get(userinput, 1)));// new af2 pre/* inserted before */
								_result_Array_string _t3 = os__read_lines(/*autofree arg*/_arg_expr_os_read_lines_1_634);
				if (_t3.is_error) {
					IError err = _t3.err;
string _arg_expr_println_1_670 = str_intp(2, _MOV((StrIntpData[]){{_SLIT("Error while loading! "), 0xfe10, {.d_s = IError_str(err)}}, {_SLIT0, 0, { .d_c = 0 }}}));// new af2 pre/* inserted before */
										println(/*autofree arg*/_arg_expr_println_1_670);
						string_free(&_arg_expr_println_1_670); // autofreed var main false
	string_free(&_arg_expr_split_0_155); // autofreed var main false
	Array_string_free(&userinput); // autofreed var main false
return;
					string_free(&_arg_expr_println_1_670); // autofreed var main false
}
				
 				Array_string content =  (*(Array_string*)_t3.data);
				map_clear(&buffer);
				int counter = 0;
				for (int _t4 = 0; _t4 < content.len; ++_t4) {
					string i = ((string*)content.data)[_t4];
					if ((i).len != 0) {
						map_set(&buffer, &(int[]){counter}, &(string[]) {  string_clone_static(i) });
						counter += _const_main__line_sep;
					}
				}
				string_free(&_arg_expr_os_read_lines_1_634); // autofreed var main false
}
			else if (string__eq(_t1, _SLIT("SAVE"))) {
				Array_int keys = map_keys(&buffer);
				Array_string content = __new_array_with_default(0, 0, sizeof(string), 0);
				if (keys.len > 0) { qsort(keys.data, keys.len, keys.element_size, (int (*)(const void *, const void *))&compare_6138052225949578915_int); };
				for (int _t5 = 0; _t5 < keys.len; ++_t5) {
					int i = ((int*)keys.data)[_t5];
					if (((*(string*)map_get(ADDR(map, buffer), &(int[]){i}, &(string[]){ (string){.str=(byteptr)"", .is_lit=1} }))).len != 0) {
						array_push((array*)&content, _MOV((string[]){ string_clone((*(string*)map_get((map*)&buffer, &(int[]){i}, &(string[]){ (string){.str=(byteptr)"", .is_lit=1} }))) }));
					}
				}
string _arg_expr_os_write_lines_1_1080 = /*2*/string_clone((*(string*)array_get(userinput, 1)));// new af2 pre/* inserted before */
								_result_void _t7 = os__write_lines(/*autofree arg*/_arg_expr_os_write_lines_1_1080, content);
				if (_t7.is_error) {
					IError err = _t7.err;
string _arg_expr_println_1_1126 = str_intp(2, _MOV((StrIntpData[]){{_SLIT("Error while saving! "), 0xfe10, {.d_s = IError_str(err)}}, {_SLIT0, 0, { .d_c = 0 }}}));// new af2 pre/* inserted before */
										println(/*autofree arg*/_arg_expr_println_1_1126);
						string_free(&_arg_expr_println_1_1126); // autofreed var main false
	Array_string_free(&content); // autofreed var main false
	array_free(&keys); // autofreed var main false
	string_free(&_arg_expr_split_0_155); // autofreed var main false
	Array_string_free(&userinput); // autofreed var main false
return;
 ...
 ;
				string_free(&_arg_expr_os_write_lines_1_1080); // autofreed var main false
	Array_string_free(&content); // autofreed var main false
	array_free(&keys); // autofreed var main false
}
			else if (string__eq(_t1, _SLIT("NEW"))) {
				map_clear(&buffer);
			}
			else if (string__eq(_t1, _SLIT("EXIT"))) {
				map_clear(&buffer);
					string_free(&_arg_expr_split_0_155); // autofreed var main false
	Array_string_free(&userinput); // autofreed var main false
break;
			}
			else if (string__eq(_t1, _SLIT(""))) {
			}
			else {
string _arg_expr_println_1_1304 = str_intp(2, _MOV((StrIntpData[]){{_SLIT("? Unknown command! "), 0xfe10, {.d_s = /*2*/string_clone(((string*)userinput.data)[0])}}, {_SLIT0, 0, { .d_c = 0 }}}));// new af2 pre/* inserted before */
								println(/*autofree arg*/_arg_expr_println_1_1304);
				string_free(&_arg_expr_println_1_1304); // autofreed var main false
}
		}
		string_free(&_arg_expr_split_0_155); // autofreed var main false
	Array_string_free(&userinput); // autofreed var main false
...
void _vcleanup(void) {
	...
	Array_string_free(&_const_os__args);
	string_free(&_const_os__wd_at_startup);
	Array_string_free(&_const_os__executable_suffixes);
